...
with st.sidebar:
    with st.expander("📚 Diccionario Morse", expanded=False):
        # Crear tabla de referencia en dos columnas
        morse_items = list(MORSE_CODE.items())
        
        # Letras A-Z
        st.markdown("**Letras A-Z** *(Click para añadir)*")
        letters = [(char, morse) for char, morse in morse_items if char.isalpha()]
        
        # Dividir letras en dos columnas
        mid_point = len(letters) // 2
        col_dict1, col_dict2 = st.columns(2)
        
        with col_dict1:
            for char, morse in letters[:mid_point]:
                if st.button(f"{char}: {morse}", key=f"letter_{char}", help=f"Añadir '{char}' al texto"):
        ...
        # Números 0-9
        st.markdown("**Números 0-9** *(Click para añadir)*")
        numbers = [(char, morse) for char, morse in morse_items if char.isdigit()]
        
        # Números en dos columnas
        mid_numbers = len(numbers) // 2
        col_num1, col_num2 = st.columns(2)
        
        with col_num1:
            for char, morse in numbers[:mid_numbers]:
                if st.button(f"{char}: {morse}", key=f"number_{char}", help=f"Añadir '{char}' al texto"):
                    if 'text_input' not in st.session_state:
                        st.session_state.text_input = ""
                    st.session_state.text_input += char
                    st.rerun()
        ...
        st.markdown("**Puntuación** *(Click para añadir)*")
        punctuation = [(char, morse) for char, morse in morse_items if not char.isalnum() and char != ' ']
        
        # Puntuación en dos columnas
        mid_punct = len(punctuation) // 2
        col_punct1, col_punct2 = st.columns(2)
        
        with col_punct1:
            for char, morse in punctuation[:mid_punct]:
                display_char = char if char != '/' else 'ESPACIO'
                if st.button(f"{display_char}: {morse}", key=f"punct_{char}", help=f"Añadir '{char}' al texto"):
                    if 'text_input' not in st.session_state:
                        st.session_state.text_input = ""
                    # Para el espacio, añadir un espacio real
                    char_to_add = ' ' if char == '/' else char
                    st.session_state.text_input += char_to_add
                    st.rerun()
        ...
        # Botón especial para añadir espacio
        st.markdown("---")
        if st.button("➕ Añadir ESPACIO", key="add_space", help="Añadir espacio al texto"):
            if 'text_input' not in st.session_state:
                st.session_state.text_input = ""
            st.session_state.text_input += " "
            st.rerun()
